...
@Component({
  ...
  template: `
      ...
      <div class="flex items-center gap-2 justify-between w-full">
        ...
        <!-- <button class="text-sm px-3 py-1 rounded-md border hover:bg-slate-50" (click)="createNewWorkflow()">New</button> -->
        
   
        
        <!-- <button 
          class="text-sm px-3 py-1 rounded-md border"
          [class.bg-indigo-50]="workflowService.isConnectMode()"
          [class.border-indigo-300]="workflowService.isConnectMode()"
          [class.hover:bg-slate-50]="!workflowService.isConnectMode()"
          (click)="workflowService.isConnectMode.set(!workflowService.isConnectMode())">
...
export class WorkflowHeaderComponent {
  ...
  getSaveButtonText(): string {
    ...
    // Check if it's a new workflow (no ID or ID is 'workflow' or 'new')
  ...
  getWorkflowDisplayName(): string {
    ...
    // Try to find the workflow name from templates
  ...
  onFileSelected(event: Event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Import the workflow using the service
        this.workflowService.importJson(workflowData);
        
        // Check if nodes were actually loaded
