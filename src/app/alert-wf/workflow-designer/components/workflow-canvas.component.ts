...
export class WorkflowCanvasComponent {
    ...
    onMouseDownNode(event: MouseEvent, id: string): void {
        ...
        // Account for pan offset and zoom when calculating offset
    ...
    onCanvasMouseDown(event: MouseEvent): void {
        ...
        // Cancel connection mode and clear selection
        if (this.workflowService.isConnectMode()) {
            this.workflowService.cancelConnection();
        }
    ...
    onCanvasContextMenu(event: MouseEvent): void {
        // Prevent default context menu when in connection mode
        if (this.workflowService.isConnectMode()) {
            event.preventDefault();
        }
    }
    ...
    onNodeDoubleClick(nodeId: string): void {
        // Double-click now only selects the node (no connection logic)
    ...
    onCanvasWheel(event: WheelEvent): void {
        ...
        const panX = this.workflowService.panOffsetX();
        const panY = this.workflowService.panOffsetY();
        // Convert from screen to world coordinates given transform: translate(pan) scale(zoom)
    ...
    private setupMouseListeners(): void {
        const onMove = (ev: MouseEvent) => {
            ...
            if (draggingId) {
                ...
                // Account for pan offset and zoom when calculating node position
                const zoom = this.workflowService.zoom();
                const panX = this.workflowService.panOffsetX();
                const panY = this.workflowService.panOffsetY();

                const x = (ev.clientX - rect.left - panX) / zoom - dx;
                const y = (ev.clientY - rect.top - panY) / zoom - dy;

                this.workflowService.updateNodePosition(draggingId, x, y);
                return;
            }
