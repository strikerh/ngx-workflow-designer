...
export class WorkflowPaletteComponent {
  ...
  private libConfig = inject(WORKFLOW_LIB_CONFIG, { optional: true }) as WorkflowDesignerLibConfig | undefined;
  ...
  private getNodesForCategory(category: PaletteCategoryConfig) {
    const prefixes = Array.isArray(category.filterPrefix) ? category.filterPrefix : [category.filterPrefix];

    return this.workflowService
      .PALETTE()
      .filter((item) => prefixes.some((prefix) => item.type.startsWith(prefix)));
  }
